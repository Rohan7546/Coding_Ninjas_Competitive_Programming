...
//	debugMode();
	...
	ll n, k1, k2;
	cin >> n >> k1 >> k2; //x=reverse, y=toggle
	ll a[n];
	for (int i = 0; i < n; i++) {cin >> a[i];}
	ll i = *max_element(a, a + n);
	ll j = *min_element(a, a + n);
	ll ans = LONG_MAX;
	for (int x = j; x <= i; x++) {
		ll increasing = 0;
		ll decreasing = 0;
		...
		//bcout << decreasing << " " << increasing << endl;
		if (decreasing > increasing) {continue;}
		ans = min(ans, decreasing * k1 + (increasing - decreasing) * k2);
	}
	cout << ans << endl;
